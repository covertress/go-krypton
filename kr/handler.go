...
import (
	...
	"github.com/krypton/go-krypton/kr/fetcher"
...
// NewProtocolManager returns a new krypton sub protocol manager. The Krypton sub protocol manages peers capable
// with the krypton network.
func NewProtocolManager(fastSync bool, networkId int, mux *event.TypeMux, txpool txPool, pow pow.PoW, blockchain *core.BlockChain, chaindb krdb.Database) (*ProtocolManager, error) {
	// Figure out whkrypton to allow fast sync or not
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	// Unregister the peer from the downloader and Krypton peer set
...
func (pm *ProtocolManager) Stop() {
	...
	glog.V(logger.Info).Infoln("Stopping krypton protocol handler...")
	...
	glog.V(logger.Info).Infoln("Krypton protocol handler stopped")
}
...
// handle is the callback invoked to manage the life cycle of an kr peer. When
// this function terminates, the peer is disconnected.
func (pm *ProtocolManager) handle(p *peer) error {
	glog.V(logger.Debug).Infof("%v: peer connected [%s]", p, p.Name())

	// Execute the Krypton handshake
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version < kr62 && msg.Code == GetBlockHashesMsg:
	...
	case p.version < kr62 && msg.Code == GetBlockHashesFromNumberMsg:
	...
	case p.version < kr62 && msg.Code == BlockHashesMsg:
	...
	case p.version < kr62 && msg.Code == GetBlocksMsg:
	...
	case p.version < kr62 && msg.Code == BlocksMsg:
	...
	case p.version >= kr62 && msg.Code == GetBlockHeadersMsg:
	...
	case p.version >= kr62 && msg.Code == BlockHeadersMsg:
	...
	case p.version >= kr62 && msg.Code == GetBlockBodiesMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= kr62 && msg.Code == BlockBodiesMsg:
	...
	case p.version >= kr63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= kr63 && msg.Code == NodeDataMsg:
	...
	case p.version >= kr63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= kr63 && msg.Code == ReceiptsMsg:
...
// KrNodeInfo represents a short summary of the Krypton sub-protocol metadata known
// about the host peer.
type KrNodeInfo struct {
	Network    int      `json:"network"`    // Krypton network ID (0=Olympic, 1=Frontier, 2=Morden)
...
func (self *ProtocolManager) NodeInfo() *KrNodeInfo {
	return &KrNodeInfo{
