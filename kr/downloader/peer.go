...
// Hash and block fetchers belonging to kr/61 and below
type relativeHashFetcherFn func(common.Hash) error
type absoluteHashFetcherFn func(uint64, int) error
type blockFetcherFn func([]common.Hash) error

// Block header and body fetchers belonging to kr/62 and above
...
type peer struct {
	...
	getRelHashes relativeHashFetcherFn // [kr/61] Method to retrieve a batch of hashes from an origin hash
	getAbsHashes absoluteHashFetcherFn // [kr/61] Method to retrieve a batch of hashes from an absolute position
	getBlocks    blockFetcherFn        // [kr/61] Method to retrieve a batch of blocks

	getRelHeaders  relativeHeaderFetcherFn // [kr/62] Method to retrieve a batch of headers from an origin hash
	getAbsHeaders  absoluteHeaderFetcherFn // [kr/62] Method to retrieve a batch of headers from an absolute position
	getBlockBodies blockBodyFetcherFn      // [kr/62] Method to retrieve a batch of block bodies

	getReceipts receiptFetcherFn // [kr/63] Method to retrieve a batch of block transaction receipts
	getNodeData stateFetcherFn   // [kr/63] Method to retrieve a batch of state trie data

	version int // Kr protocol version number to switch strategies
	lock    sync.RWMutex
}
...
func (p *peer) Fetch61(request *fetchRequest) error {
	...
	if p.version != 61 {
		panic(fmt.Sprintf("block fetch [kr/61] requested on kr/%d", p.version))
	}
...
func (p *peer) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [kr/62+] requested on kr/%d", p.version))
	}
	// Short circuit if the peer is already fetching
	if !atomic.CompareAndSwapInt32(&p.blockIdle, 0, 1) {
		return errAlreadyFetching
...
func (p *peer) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [kr/63+] requested on kr/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peer) FetchNodeData(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [kr/63+] requested on kr/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
// Lacks retrieves whkrypton the hash of a blockchain item is on the peers lacking
// list (i.e. whkrypton we know that the peer does not have it).
