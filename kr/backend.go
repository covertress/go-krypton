...
func New(config *Config) (*Krypton, error) {
	...
	kr := &Krypton{
		...
		kryptonbase:               config.Kryptonbase,
...
func (s *Krypton) Kryptonbase() (eb common.Address, err error) {
	eb = s.kryptonbase
	if (eb == common.Address{}) {
		addr, e := s.AccountManager().AddressByIndex(0)
		if e != nil {
			err = fmt.Errorf("kryptonbase address must be explicitly specified")
		}
...
func (self *Krypton) SetKryptonbase(kryptonbase common.Address) {
	self.kryptonbase = kryptonbase
	self.miner.SetKryptonbase(kryptonbase)
}
