...
// PeerInfo represents a short summary of the Krypton sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Krypton protocol version negotiated
...
// Handshake executes the kr protocol handshake, negotiating version number,
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("kr/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Krypton sub-protocol.
type peerSet struct {
