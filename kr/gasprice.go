...
func NewGasPriceOracle(kr *Krypton) *GasPriceOracle {
	minprice := kr.GpoMinGasPrice
	if minprice == nil {
		minprice = big.NewInt(gpoDefaultMinGasPrice)
	}
	minbase := new(big.Int).Mul(minprice, big.NewInt(100))
	if kr.GpobaseCorrectionFactor > 0 {
		minbase = minbase.Div(minbase, big.NewInt(int64(kr.GpobaseCorrectionFactor)))
	}
...
func (gpo *GasPriceOracle) init() {
	gpo.initOnce.Do(func() {
		gpo.processPastBlocks(gpo.kr.BlockChain())
		go gpo.listenLoop()
	})
}
...
func (self *GasPriceOracle) listenLoop() {
	events := self.kr.EventMux().Subscribe(core.ChainEvent{}, core.ChainSplitEvent{})
...
func (self *GasPriceOracle) processBlock(block *types.Block) {
	...
	if lastBase.Cmp(lp) < 0 {
		corr = self.kr.GpobaseStepUp
	} else {
		corr = -self.kr.GpobaseStepDown
	}
...
func (self *GasPriceOracle) lowestPrice(block *types.Block) *big.Int {
	...
	receipts := core.GetBlockReceipts(self.kr.ChainDb(), block.Hash())
	if len(receipts) > 0 {
	...
	if new(big.Int).Mul(gasUsed, big.NewInt(100)).Cmp(new(big.Int).Mul(block.GasLimit(),
		big.NewInt(int64(self.kr.GpoFullBlockRatio)))) < 0 {
...
func (self *GasPriceOracle) SuggestPrice() *big.Int {
	...
	price.Mul(price, big.NewInt(int64(self.kr.GpobaseCorrectionFactor)))
	price.Div(price, big.NewInt(100))
	if price.Cmp(self.minPrice) < 0 {
		price.Set(self.minPrice)
	} else if self.kr.GpoMaxGasPrice != nil && price.Cmp(self.kr.GpoMaxGasPrice) > 0 {
		price.Set(self.kr.GpoMaxGasPrice)
	}
	return price
}
