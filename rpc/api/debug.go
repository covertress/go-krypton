...
func NewDebugApi(xkr *xkr.XKr, krypton *kr.Krypton, coder codec.Codec) *debugApi {
	return &debugApi{
		xkr:     xkr,
		krypton: krypton,
...
func (self *debugApi) DumpBlock(req *shared.Request) (interface{}, error) {
	...
	stateDb, err := state.New(block.Root(), self.krypton.ChainDb())
	if err != nil {
		return nil, err
	}
...
func (self *debugApi) SetHead(req *shared.Request) (interface{}, error) {
	...
	self.krypton.BlockChain().SetHead(uint64(args.BlockNumber))
...
func (self *debugApi) ProcessBlock(req *shared.Request) (interface{}, error) {
	...
	var (
		blockchain = self.krypton.BlockChain()
	...
	statedb, err := state.New(blockchain.GetBlock(block.ParentHash()).Root(), self.krypton.ChainDb())
	if err != nil {
		return false, err
	}
...
func (self *debugApi) SeedHash(req *shared.Request) (interface{}, error) {
	...
	if hash, err := krash.GetSeedHash(uint64(args.BlockNumber)); err == nil {
