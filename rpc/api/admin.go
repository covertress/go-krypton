...
func NewAdminApi(xkr *xkr.XKr, krypton *kr.Krypton, codec codec.Codec) *adminApi {
	return &adminApi{
		xkr:     xkr,
		krypton: krypton,
		codec:    codec,
...
func (self *adminApi) AddPeer(req *shared.Request) (interface{}, error) {
	...
	err := self.krypton.AddPeer(args.Url)
	if err == nil {
		return true, nil
	}
	return false, err
...
func (self *adminApi) Peers(req *shared.Request) (interface{}, error) {
	return self.krypton.Network().PeersInfo(), nil
}

func (self *adminApi) NodeInfo(req *shared.Request) (interface{}, error) {
	return self.krypton.Network().NodeInfo(), nil
}

func (self *adminApi) DataDir(req *shared.Request) (interface{}, error) {
	return self.krypton.DataDir, nil
}
...
func (self *adminApi) ImportChain(req *shared.Request) (interface{}, error) {
	...
	for batch := 0; ; batch++ {
		...
		// Import the batch.
		if hasAllBlocks(self.krypton.BlockChain(), blocks[:i]) {
			continue
		}
		if _, err := self.krypton.BlockChain().InsertChain(blocks[:i]); err != nil {
...
func (self *adminApi) ExportChain(req *shared.Request) (interface{}, error) {
	...
	defer fh.Close()
	if err := self.krypton.BlockChain().Export(fh); err != nil {
		return false, err
	}
...
func (self *adminApi) SetSolc(req *shared.Request) (interface{}, error) {
	...
	solc, err := self.xkr.SetSolc(args.Path)
	if err != nil {
		return nil, err
	}
...
func (self *adminApi) StartRPC(req *shared.Request) (interface{}, error) {
	...
	apis, err := ParseApiString(args.Apis, self.codec, self.xkr, self.krypton)
	if err != nil {
		return false, err
	}
...
func (self *adminApi) SleepBlocks(req *shared.Request) (interface{}, error) {
	...
	height = new(big.Int).Add(self.xkr.CurrentBlock().Number(), big.NewInt(args.N))
	height, err = sleepBlocks(self.xkr.UpdateState(), height, timer)
	if err != nil {
		return nil, err
	}
...
func (self *adminApi) Register(req *shared.Request) (interface{}, error) {
	...
	codeb := self.xkr.CodeAtBytes(args.Address)
	codeHash := common.BytesToHash(crypto.Sha3(codeb))
	contentHash := common.HexToHash(args.ContentHashHex)
	registry := registrar.New(self.xkr)
...
func (self *adminApi) GetContractInfo(req *shared.Request) (interface{}, error) {
	...
	infoDoc, err := natspec.FetchDocsForContract(args.Contract, self.xkr, self.krypton.HTTPClient())
	if err != nil {
		return nil, err
	}
...
func (self *adminApi) HttpGet(req *shared.Request) (interface{}, error) {
	...
	resp, err := self.krypton.HTTPClient().Get(args.Uri, args.Path)
	if err != nil {
		return nil, err
	}
...
func (self *adminApi) EnableUserAgent(req *shared.Request) (interface{}, error) {
	if fe, ok := self.xkr.Frontend().(*useragent.RemoteFrontend); ok {
		fe.Enable()
	}
	return true, nil
}
