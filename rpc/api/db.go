...
func NewDbApi(xkr *xkr.XKr, krypton *kr.Krypton, coder codec.Codec) *dbApi {
	return &dbApi{
		xkr:     xkr,
		krypton: krypton,
		methods:  DbMapping,
...
func (self *dbApi) GetString(req *shared.Request) (interface{}, error) {
	...
	ret, err := self.xkr.DbGet([]byte(args.Database + args.Key))
...
func (self *dbApi) PutString(req *shared.Request) (interface{}, error) {
	...
	return self.xkr.DbPut([]byte(args.Database+args.Key), args.Value), nil
}
...
func (self *dbApi) GetHex(req *shared.Request) (interface{}, error) {
	...
	if res, err := self.xkr.DbGet([]byte(args.Database + args.Key)); err == nil {
