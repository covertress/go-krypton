...
var (
	...
	MinerMapping = map[string]minerhandler{
		...
		"miner_setKryptonbase": (*minerApi).SetKryptonbase,
...
func (self *minerApi) SetKryptonbase(req *shared.Request) (interface{}, error) {
	args := new(SetKryptonbaseArgs)
	if err := self.codec.Decode(req.Params, &args); err != nil {
		return false, err
	}
	self.krypton.SetKryptonbase(args.Kryptonbase)
	return nil, nil
}
