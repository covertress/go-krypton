...
type SetKryptonbaseArgs struct {
	Kryptonbase common.Address
}

func (args *SetKryptonbaseArgs) UnmarshalJSON(b []byte) (err error) {
	var obj []interface{}
	if err := json.Unmarshal(b, &obj); err != nil {
		return shared.NewDecodeParamError(err.Error())
	}
	...
	if addr, ok := obj[0].(string); ok {
		args.Kryptonbase = common.HexToAddress(addr)
		if (args.Kryptonbase == common.Address{}) {
			return shared.NewInvalidTypeError("Kryptonbase", "not a valid address")
		}
		return nil
	}

	return shared.NewInvalidTypeError("Kryptonbase", "not a string")
}
