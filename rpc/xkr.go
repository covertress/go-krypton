...
// Package rpc implements the Krypton JSON-RPC API.
package rpc
...
// Xkr is a native API interface to a remote node.
type Xkr struct {
	client comms.KryptonClient
	reqId  uint32
}

// NewXkr constructs a new native API interface to a remote node.
func NewXkr(client comms.KryptonClient) *Xkr {
	return &Xkr{
		client: client,
	}
}
...
func (self *Xkr) Call(method string, params []interface{}) (map[string]interface{}, error) {
