...
// Package xkr is the interface to all Krypton functionality.
package xkr
...
import (
	...
	"github.com/krypton/go-krypton/kr/filters"
...
type XKr struct {
	...
	gpo           *kr.GasPriceOracle
	state         *State
	whisper       *Whisper
	filterManager *filters.FilterSystem
}

func NewTest(kr *kr.Krypton, frontend Frontend) *XKr {
	return &XKr{backend: kr, frontend: frontend}
}

// New creates an XKr that uses the given frontend.
// If a nil Frontend is provided, a default frontend which
// confirms all transactions will be used.
func New(krypton *kr.Krypton, frontend Frontend) *XKr {
	xkr := &XKr{
		backend:          krypton,
		frontend:         frontend,
		quit:             make(chan struct{}),
		filterManager:    filters.NewFilterSystem(krypton.EventMux()),
		logQueue:         make(map[int]*logQueue),
		blockQueue:       make(map[int]*hashQueue),
		transactionQueue: make(map[int]*hashQueue),
		messages:         make(map[int]*whisperFilter),
		agent:            miner.NewRemoteAgent(),
		gpo:              kr.NewGasPriceOracle(krypton),
	}
	if krypton.Whisper() != nil {
		xkr.whisper = NewWhisper(krypton.Whisper())
	}
	krypton.Miner().Register(xkr.agent)
	if frontend == nil {
		xkr.frontend = dummyFrontend{}
	}
	state, _ := xkr.backend.BlockChain().State()
	xkr.state = NewState(xkr, state)
	go xkr.start()
	return xkr
}
...
func (self *XKr) DefaultGasPrice() *big.Int {
	return self.gpo.SuggestPrice()
}

func (self *XKr) RemoteMining() *miner.RemoteAgent { return self.agent }

func (self *XKr) AtStateNum(num int64) *XKr {
...
func (self *XKr) WithState(statedb *state.StateDB) *XKr {
	xkr := &XKr{
	...
	xkr.state = NewState(xkr, statedb)
	return xkr
}

func (self *XKr) State() *State { return self.state }
...
func (self *XKr) UpdateState() (wait chan *big.Int) {
...
func (self *XKr) Whisper() *Whisper { return self.whisper }

func (self *XKr) getBlockByHeight(height int64) *types.Block {
	var num uint64
...
func (self *XKr) BlockByHash(strHash string) *Block {
...
func (self *XKr) KrBlockByHash(strHash string) *types.Block {
	hash := common.HexToHash(strHash)
	block := self.backend.BlockChain().GetBlock(hash)
...
func (self *XKr) KrTransactionByHash(hash string) (*types.Transaction, common.Hash, uint64, uint64) {
...
func (self *XKr) BlockByNumber(num int64) *Block {
	return NewBlock(self.getBlockByHeight(num))
}

func (self *XKr) KrBlockByNumber(num int64) *types.Block {
	return self.getBlockByHeight(num)
}

func (self *XKr) Td(hash common.Hash) *big.Int {
	return self.backend.BlockChain().GetTd(hash)
}

func (self *XKr) CurrentBlock() *types.Block {
	return self.backend.BlockChain().CurrentBlock()
}

func (self *XKr) GetBlockReceipts(bhash common.Hash) types.Receipts {
	return core.GetBlockReceipts(self.backend.ChainDb(), bhash)
}

func (self *XKr) GetTxReceipt(txhash common.Hash) *types.Receipt {
	return core.GetReceipt(self.backend.ChainDb(), txhash)
}

func (self *XKr) GasLimit() *big.Int {
	return self.backend.BlockChain().GasLimit()
}

func (self *XKr) Block(v interface{}) *Block {
	if n, ok := v.(int32); ok {
...
func (self *XKr) Solc() (*compiler.Solidity, error) {
	return self.backend.Solc()
}

// set in js console via admin interface or wrapper from cli flags
func (self *XKr) SetSolc(solcPath string) (*compiler.Solidity, error) {
	self.backend.SetSolc(solcPath)
	return self.Solc()
}

// store DApp value in extra database
func (self *XKr) DbPut(key, val []byte) bool {
	self.backend.DappDb().Put(append(dappStorePre, key...), val)
	return true
}

// retrieve DApp value from extra database
func (self *XKr) DbGet(key []byte) ([]byte, error) {
	val, err := self.backend.DappDb().Get(append(dappStorePre, key...))
	return val, err
}

func (self *XKr) PeerCount() int {
	return self.backend.PeerCount()
}

func (self *XKr) IsMining() bool {
	return self.backend.IsMining()
}

func (self *XKr) HashRate() int64 {
	return self.backend.Miner().HashRate()
}

func (self *XKr) KrVersion() string {
	return fmt.Sprintf("%d", self.backend.KrVersion())
}

func (self *XKr) NetworkVersion() string {
	return fmt.Sprintf("%d", self.backend.NetVersion())
}

func (self *XKr) WhisperVersion() string {
	return fmt.Sprintf("%d", self.backend.ShhVersion())
}

func (self *XKr) ClientVersion() string {
	return self.backend.ClientVersion()
}

func (self *XKr) SetMining(shouldmine bool, threads int) bool {
...
func (self *XKr) IsListening() bool {
	return self.backend.IsListening()
}

func (self *XKr) Coinbase() string {
	eb, err := self.backend.Kryptonbase()
	if err != nil {
		return "0x0"
	}
	return eb.Hex()
...
func (self *XKr) NewLogFilter(earliest, latest int64, skip, max int, address []string, topics [][]string) int {
	self.logMu.Lock()
	defer self.logMu.Unlock()
...
func (self *XKr) NewTransactionFilter() int {
	self.transactionMu.Lock()
	defer self.transactionMu.Unlock()
...
func (self *XKr) NewBlockFilter() int {
	self.blockMu.Lock()
	defer self.blockMu.Unlock()
...
func (self *XKr) GetFilterType(id int) byte {
	if _, ok := self.blockQueue[id]; ok {
...
func (self *XKr) LogFilterChanged(id int) vm.Logs {
	self.logMu.Lock()
	defer self.logMu.Unlock()
...
func (self *XKr) BlockFilterChanged(id int) []common.Hash {
	self.blockMu.Lock()
	defer self.blockMu.Unlock()
...
func (self *XKr) TransactionFilterChanged(id int) []common.Hash {
	self.blockMu.Lock()
	defer self.blockMu.Unlock()
...
func (self *XKr) Logs(id int) vm.Logs {
...
func (self *XKr) AllLogs(earliest, latest int64, skip, max int, address []string, topics [][]string) vm.Logs {
	filter := filters.New(self.backend.ChainDb())
	filter.SetBeginBlock(earliest)
	filter.SetEndBlock(latest)
	filter.SetAddresses(cAddress(address))
...
// HEX encoded.
func (p *XKr) NewWhisperFilter(to, from string, topics [][]string) int {
	// Pre-define the id to be filled later
	var id int

	// Callback to delegate core whisper messages to this xkr filter
	...
	// Initialize the core whisper filter and wrap into xkr
...
func (p *XKr) UninstallWhisperFilter(id int) bool {
	p.messagesMu.Lock()
...
func (self *XKr) WhisperMessages(id int) []WhisperMessage {
...
func (self *XKr) WhisperMessagesChanged(id int) []WhisperMessage {
	self.messagesMu.RLock()
	defer self.messagesMu.RUnlock()
...
// func (self *XKr) Register(args string) bool {
// 	self.regmut.Lock()
...
// func (self *XKr) Unregister(args string) bool {
// 	self.regmut.Lock()
// 	defer self.regmut.Unlock()
...
// func (self *XKr) PullWatchTx(args string) []*interface{} {
// 	self.regmut.Lock()
// 	defer self.regmut.Unlock()
...
func (self *XKr) EachStorage(addr string) string {
	var values []KeyVal
...
func (self *XKr) ToAscii(str string) string {
	padded := common.RightPadBytes([]byte(str), 32)

	return "0x" + common.ToHex(padded)
}

func (self *XKr) FromAscii(str string) string {
	if common.IsHex(str) {
		str = str[2:]
	}
...
func (self *XKr) FromNumber(str string) string {
	if common.IsHex(str) {
		str = str[2:]
	}
...
func (self *XKr) PushTx(encodedTx string) (string, error) {
...
func (self *XKr) Call(fromStr, toStr, valueStr, gasStr, gasPriceStr, dataStr string) (string, string, error) {
...
func (self *XKr) ConfirmTransaction(tx string) bool {
	return self.frontend.ConfirmTransaction(tx)
}

func (self *XKr) doSign(from common.Address, hash common.Hash, didUnlock bool) ([]byte, error) {
...
func (self *XKr) Sign(fromStr, hashStr string, didUnlock bool) (string, error) {
	var (
...
func (self *XKr) Frontend() Frontend {
	return self.frontend
}

func (self *XKr) SignTransaction(fromStr, toStr, nonceStr, valueStr, gasStr, gasPriceStr, codeStr string) (*types.Transaction, error) {
...
func (self *XKr) Transact(fromStr, toStr, nonceStr, valueStr, gasStr, gasPriceStr, codeStr string) (string, error) {
...
func (self *XKr) sign(tx *types.Transaction, from common.Address, didUnlock bool) (*types.Transaction, error) {
	hash := tx.SigHash()
	sig, err := self.doSign(from, hash, didUnlock)
	if err != nil {
		return tx, err
