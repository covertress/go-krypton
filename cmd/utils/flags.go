...
import (
	...
	"github.com/krypton/go-krypton/rpc/useragent"
...
var (
	...
	KryptonbaseFlag = cli.StringFlag{
		Name:  "kryptonbase",
...
// MakeKrConfig creates krypton options from set command line flags.
func MakeKrConfig(clientID, version string, ctx *cli.Context) *kr.Config {
	customName := ctx.GlobalString(IdentityFlag.Name)
	if len(customName) > 0 {
		clientID += "/" + customName
	}
	am := MakeAccountManager(ctx)
	kryptonbase, err := ParamToAddress(ctx.GlobalString(KryptonbaseFlag.Name), am)
	if err != nil {
		glog.V(logger.Error).Infoln("WARNING: No kryptonbase set and no accounts found as default")
	}
	// Assemble the entire kr configuration and return
	cfg := &kr.Config{
		...
		Kryptonbase:               common.HexToAddress(kryptonbase),
	...
	if ctx.GlobalBool(DevModeFlag.Name) {
		...
		if !ctx.GlobalIsSet(DataDirFlag.Name) {
			cfg.DataDir = os.TempDir() + "/krypton_dev_mode"
		}
		cfg.PowTest = true
		cfg.DevMode = true
...
func MakeChain(ctx *cli.Context) (chain *core.BlockChain, chainDb krdb.Database) {
	datadir := MustDataDir(ctx)
	cache := ctx.GlobalInt(CacheFlag.Name)

	var err error
	if chainDb, err = krdb.NewLDBDatabase(filepath.Join(datadir, "chaindata"), cache); err != nil {
...
func IpcSocketPath(ctx *cli.Context) (ipcpath string) {
	if runtime.GOOS == "windows" {
		...
		ipcpath = common.DefaultIpcPath()
		if ctx.GlobalIsSet(DataDirFlag.Name) {
			ipcpath = filepath.Join(ctx.GlobalString(DataDirFlag.Name), "gkr.ipc")
		}
		if ctx.GlobalIsSet(IPCPathFlag.Name) {
			ipcpath = ctx.GlobalString(IPCPathFlag.Name)
		}
...
func StartIPC(kr *kr.Krypton, ctx *cli.Context) error {
	config := comms.IpcConfig{
		Endpoint: IpcSocketPath(ctx),
	}

	initializer := func(conn net.Conn) (comms.Stopper, shared.KryptonApi, error) {
		fe := useragent.NewRemoteFrontend(conn, kr.AccountManager())
		xkr := xkr.New(kr, fe)
		apis, err := api.ParseApiString(ctx.GlobalString(IPCApiFlag.Name), codec.JSON, xkr, kr)
		if err != nil {
			return nil, nil, err
		}
		return xkr, api.Merge(apis...), nil
	}

	return comms.StartIpc(config, codec.JSON, initializer)
}

func StartRPC(kr *kr.Krypton, ctx *cli.Context) error {
	...
	apis, err := api.ParseApiString(ctx.GlobalString(RpcApiFlag.Name), codec, xkr, kr)
	if err != nil {
		return err
	}
