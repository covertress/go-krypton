...
func MakeKrConfig(clientID, version string, ctx *cli.Context) *kr.Config {
	...
	krbase, err := ParamToAddress(ctx.GlobalString(KrbaseFlag.Name), am)
	if err != nil {
		glog.V(logger.Error).Infoln("WARNING: No krbase set and no accounts found as default")
	}
	...
	cfg := &kr.Config{
		...
		Krbase:               common.HexToAddress(krbase),
