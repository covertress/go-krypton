...
// Package utils contains internal helper functions for go-krypton commands.
package utils
...
func StartKrypton(krypton *kr.Krypton) {
	glog.V(logger.Info).Infoln("Starting", krypton.Name())
	if err := krypton.Start(); err != nil {
		Fatalf("Error starting Krypton: %v", err)
	}
	go func() {
