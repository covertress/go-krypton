...
var (
	...
	monitorCommand = cli.Command{
		Action: monitor,
		Name:   "monitor",
		Usage:  `Gkr Monitor: node metrics monitoring and visualization`,
		Description: `
The Gkr monitor is a tool to collect and visualize various internal metrics
...
func monitor(ctx *cli.Context) {
	...
	// Attach to an Krypton node over IPC or RPC
	...
	xkr := rpc.NewXkr(client)

	// Retrieve all the available metrics and resolve the user pattens
	metrics, err := retrieveMetrics(xkr)
	if err != nil {
	...
	if len(monitored) == 0 {
		...
		if len(list) > 0 {
			...
			utils.Fatalf("No metrics collected by gkr (--%s).\n", utils.MetricsEnabledFlag.Name)
		}
	}
	...
	refreshCharts(xkr, monitored, data, units, charts, ctx, footer)
	termui.Body.Align()
	...
	for {
		...
		case <-refresh:
			if refreshCharts(xkr, monitored, data, units, charts, ctx, footer) {
				termui.Body.Align()
			}
			termui.Render(termui.Body)
		}
...
// retrieveMetrics contacts the attached gkr node and retrieves the entire set
// of collected system metrics.
func retrieveMetrics(xkr *rpc.Xkr) (map[string]interface{}, error) {
	return xkr.Call("debug_metrics", []interface{}{true})
}
...
func refreshCharts(xkr *rpc.Xkr, metrics []string, data [][]float64, units []int, charts []*termui.LineChart, ctx *cli.Context, footer *termui.Par) (realign bool) {
	values, err := retrieveMetrics(xkr)
