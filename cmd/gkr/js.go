...
import (
	...
	re "github.com/krypton/go-krypton/jsre"
	"github.com/krypton/go-krypton/rpc"
	"github.com/krypton/go-krypton/rpc/api"
...
func newLightweightJSRE(docRoot string, client comms.KryptonClient, datadir string, interactive bool) *jsre {
	js := &jsre{ps1: "> "}
...
func newJSRE(krypton *kr.Krypton, docRoot, corsDomain string, client comms.KryptonClient, interactive bool, f xkr.Frontend) *jsre {
	js := &jsre{krypton: krypton, ps1: "> "}
	// set default cors domain used by startRpc from CLI flag
	js.corsDomain = corsDomain
	if f == nil {
		f = js
	}
	js.xkr = xkr.New(krypton, f)
	js.wait = js.xkr.UpdateState()
	js.client = client
	if clt, ok := js.client.(*comms.InProcClient); ok {
		if offeredApis, err := api.ParseApiString(shared.AllApis, codec.JSON, js.xkr, krypton); err == nil {
	...
	if !liner.TerminalSupported() || !interactive {
		...
		lr := liner.NewLiner()
		js.withHistory(krypton.DataDir, func(hist *os.File) { lr.ReadHistory(hist) })
		lr.SetCtrlCAborts(true)
		js.loadAutoCompletion()
		lr.SetWordCompleter(apiWordCompleter)
		lr.SetTabCompletionStyle(liner.TabPrints)
		js.prompter = lr
		js.atexit = func() {
			js.withHistory(krypton.DataDir, func(hist *os.File) { hist.Truncate(0); lr.WriteHistory(hist) })
			lr.Close()
			close(js.wait)
		}
	}
...
// show summary of current gkr instance
func (self *jsre) welcome() {
	self.re.Run(`
    (function () {
      console.log('instance: ' + web3.version.client);
      console.log(' datadir: ' + admin.datadir);
      console.log("coinbase: " + kr.coinbase);
      var ts = 1000 * kr.getBlock(kr.blockNumber).timestamp;
      console.log("at block: " + kr.blockNumber + " (" + new Date(ts) + ")");
    })();
  `)
	if modules, err := self.supportedApis(); err == nil {
...
func (js *jsre) apiBindings(f xkr.Frontend) error {
	...
	apiImpl, err := api.ParseApiString(strings.Join(apiNames, ","), codec.JSON, js.xkr, js.krypton)
	if err != nil {
		utils.Fatalf("Unable to determine supported api's: %v", err)
	}

	jkr := rpc.NewJkr(api.Merge(apiImpl...), js.re, js.client, f)
	js.re.Set("jkr", struct{}{})
	t, _ := js.re.Get("jkr")
	jkrObj := t.Object()

	jkrObj.Set("send", jkr.Send)
	jkrObj.Set("sendAsync", jkr.Send)
	...
	err = js.re.Compile("krypton.js", re.Web3_JS)
	if err != nil {
	...
	_, err = js.re.Run("web3.setProvider(jkr)")
	if err != nil {
		utils.Fatalf("Error setting web3 provider: %v", err)
	}

	// load only supported API's in javascript runtime
	shortcuts := "var kr = web3.kr; "
	...
	js.re.Run(`var GlobalRegistrar = kr.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	return nil
}
...
func (self *jsre) ConfirmTransaction(tx string) bool {
	if self.krypton.NatSpec {
		notice := natspec.GetNotice(self.xkr, tx, self.krypton.HTTPClient())
		fmt.Println(notice)
...
func (self *jsre) UnlockAccount(addr []byte) bool {
	...
	// TODO: allow retry
	if err := self.krypton.AccountManager().Unlock(common.BytesToAddress(addr), pass); err != nil {
		return false
	} else {
