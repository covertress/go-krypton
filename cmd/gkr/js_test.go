...
import (
	...
	"github.com/krypton/go-krypton/rpc/comms"
)
...
type testjkrre struct {
	*jsre
	lastConfirm string
	client      *httpclient.HTTPClient
}

func (self *testjkrre) UnlockAccount(acc []byte) bool {
	err := self.krypton.AccountManager().Unlock(common.BytesToAddress(acc), "")
	if err != nil {
...
func (self *testjkrre) ConfirmTransaction(tx string) bool {
	if self.krypton.NatSpec {
		self.lastConfirm = natspec.GetNotice(self.xkr, tx, self.client)
	}
	return true
}

func testJKrRE(t *testing.T) (string, *testjkrre, *kr.Krypton) {
	return testREPL(t, nil)
}

func testREPL(t *testing.T, config func(*kr.Config)) (string, *testjkrre, *kr.Krypton) {
	tmp, err := ioutil.TempDir("", "gkr-test")
	if err != nil {
		t.Fatal(err)
	}
	...
	conf := &kr.Config{
		...
		PowTest:        true,
		NewDB:          func(path string) (krdb.Database, error) { return db, nil },
	}
	if config != nil {
		config(conf)
	}
	krypton, err := kr.New(conf)
	if err != nil {
		t.Fatal("%v", err)
	}
	...
	assetPath := filepath.Join(os.Getenv("GOPATH"), "src", "github.com", "krypton", "go-krypton", "cmd", "mist", "assets", "ext")
	client := comms.NewInProcClient(codec.JSON)
	tf := &testjkrre{client: krypton.HTTPClient()}
	repl := newJSRE(krypton, assetPath, "", client, false, tf)
	tf.jsre = repl
	return tmp, tf, krypton
}

func TestNodeInfo(t *testing.T) {
	t.Skip("broken after p2p update")
	tmp, repl, krypton := testJKrRE(t)
	if err := krypton.Start(); err != nil {
		t.Fatalf("error starting krypton: %v", err)
	}
	defer krypton.Stop()
...
func TestAccounts(t *testing.T) {
	...
	checkEvalJSON(t, repl, `kr.accounts`, `["`+testAddress+`"]`)
	checkEvalJSON(t, repl, `kr.coinbase`, `"`+testAddress+`"`)
	...
	checkEvalJSON(t, repl, `kr.accounts`, `["`+testAddress+`","`+addr+`"]`)
...
func TestBlockChain(t *testing.T) {
	...
	val, err := repl.re.Run("JSON.stringify(debug.dumpBlock(kr.blockNumber))")
	if err != nil {
		t.Errorf("expected no error, got %v", err)
	}
	...
	// do the export
	extmp, err := ioutil.TempDir("", "gkr-test-export")
	if err != nil {
		t.Fatal(err)
	}
	...
	checkEvalJSON(t, repl, `debug.dumpBlock(kr.blockNumber)`, beforeExport)
}
...
func TestMining(t *testing.T) {
	...
	defer krypton.Stop()
	defer os.RemoveAll(tmp)
	checkEvalJSON(t, repl, `kr.mining`, `false`)
}
...
func TestRPC(t *testing.T) {
	...
	checkEvalJSON(t, repl, `admin.startRPC("127.0.0.1", 5004, "*", "web3,kr,net")`, `true`)
}
...
func TestCheckTestAccountBalance(t *testing.T) {
	...
	checkEvalJSON(t, repl, `kr.getBalance(primary)`, `"`+testBalance+`"`)
}
...
func TestSignature(t *testing.T) {
	...
	val, err := repl.re.Run(`kr.sign("` + testAddress + `", "` + testHash + `")`)
...
func TestContract(t *testing.T) {
	t.Skip("contract testing is implemented with mining in krash test mode. This takes about 7seconds to run. Unskip and run on demand")
	coinbase := common.HexToAddress(testAddress)
	tmp, repl, krypton := testREPL(t, func(conf *kr.Config) {
		conf.Kryptonbase = coinbase
		conf.PowTest = true
	})
	if err := krypton.Start(); err != nil {
		t.Errorf("error starting krypton: %v", err)
	...
	* then set once more SetHashReg SetUrlHint
	...
	if checkEvalJSON(t, repl, `primary = kr.accounts[0]`, `"`+testAddress+`"`) != nil {
		return
	}
	...
	if err != nil {
		...
		if checkEvalJSON(t, repl, `contract = kr.compile.solidity(source).test`, string(contractInfo)) != nil {
			return
		}
	}
	...
	if checkEvalJSON(
		t, repl,
		`contractaddress = kr.sendTransaction({from: primary, data: contract.code})`,
...
func pendingTransactions(repl *testjkrre, t *testing.T) (txc int64, err error) {
	txs := repl.krypton.TxPool().GetTransactions()
	return int64(len(txs)), nil
}

func processTxs(repl *testjkrre, t *testing.T, expTxc int) bool {
	var txc int64
	var err error
	for i := 0; i < 50; i++ {
...
func checkEvalJSON(t *testing.T, re *testjkrre, expr, want string) error {
