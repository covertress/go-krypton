...
// gkr is the official command-line client for Krypton.
package main
...
func init() {
	...
	app = utils.NewApp(Version, "the go-krypton command line interface")
	app.Action = run
...
`,
		...
		monitorCommand,
		{
			Action: makedag,
			Name:   "makedag",
			Usage:  "generate krash dag (for testing)",
			Description: `
The makedag command generates an krash DAG in /tmp/dag.
...
`,
		},
		{
			Action: version,
			Name:   "version",
			Usage:  "print krypton version numbers",
			Description: `
...
`,
		...
		{
			Name:  "wallet",
			Usage: "krypton presale wallet",
			Subcommands: []cli.Command{
				{
					Action: importWallet,
					Name:   "import",
					Usage:  "import krypton presale wallet",
				},
			},
			Description: `
...
will prompt for your password and imports your krypton presale account.
...
between krypton nodes by simply copying.
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    krypton --password <passwordfile> account new
...
password to file or expose in any other way.
    ...
    krypton account update <address>
...
For non-interactive use the passphrase can be specified with the --password flag:
    ...
    krypton --password <passwordfile> account update <address>
...
changes.
    ...
    krypton account import <keyfile>
...
For non-interactive use the passphrase can be specified with the -password flag:
    ...
    krypton --password <passwordfile> account import <keyfile>

Note:
As you can directly copy your encrypted accounts to another krypton instance,
...
nodes.
		...
		},
		{
			Action: console,
			Name:   "console",
			Usage:  `Gkr Console: interactive JavaScript environment`,
			Description: `
The Gkr console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/krypton/go-krypton/wiki/Javascipt-Console
`},
		{
			Action: attach,
			Name:   "attach",
			Usage:  `Gkr Console: interactive JavaScript environment (connect to node)`,
			Description: `
The Gkr console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/krypton/go-krypton/wiki/Javascipt-Console.
This command allows to open a console on a running gkr node.
`,
		},
		{
			Action: execJSFiles,
			Name:   "js",
			Usage:  `executes the given JavaScript files in the Gkr JavaScript VM`,
			Description: `
The JavaScript VM exposes a node admin interface as well as the Ðapp
JavaScript API. See https://github.com/krypton/go-krypton/wiki/Javascipt-Console
`,
		},
	}
	app.Flags = []cli.Flag{
...
func run(ctx *cli.Context) {
	cfg := utils.MakeKrConfig(ClientIdentifier, nodeNameVersion, ctx)
...
func attach(ctx *cli.Context) {
	var client comms.KryptonClient
	var err error
	if ctx.Args().Present() {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to gkr node - %v", err)
	}
...
func blockRecovery(ctx *cli.Context) {
	...
	cfg := utils.MakeKrConfig(ClientIdentifier, nodeNameVersion, ctx)
	blockDb, err := krdb.NewLDBDatabase(filepath.Join(cfg.DataDir, "blockchain"), cfg.DatabaseCache)
	if err != nil {
...
func startKr(ctx *cli.Context, kr *kr.Krypton) {
	// Start Krypton itself
	utils.StartKrypton(kr)

	am := kr.AccountManager()
	...
	if !ctx.GlobalBool(utils.IPCDisabledFlag.Name) {
		if err := utils.StartIPC(kr, ctx); err != nil {
			utils.Fatalf("Error string IPC: %v", err)
		}
	}
	if ctx.GlobalBool(utils.RPCEnabledFlag.Name) {
		if err := utils.StartRPC(kr, ctx); err != nil {
...
func makedag(ctx *cli.Context) {
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: gkr makedag <block number> <outputdir>`)
	}
	switch {
	case len(args) == 2:
...
func gpuinfo(ctx *cli.Context) {
	kr.PrintOpenCLDevices()
}

func gpubench(ctx *cli.Context) {
	args := ctx.Args()
	wrongArgs := func() {
		utils.Fatalf(`Usage: gkr gpubench <gpu number>`)
	}
	switch {
	case len(args) == 1:
...
func version(c *cli.Context) {
	...
	fmt.Println("Protocol Versions:", kr.ProtocolVersions)
