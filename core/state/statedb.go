...
// Package state provides a caching layer atop the Krypton state trie.
package state
...
// StateDBs within the krypton protocol are used to store anything
...
func New(root common.Hash, db krdb.Database) (*StateDB, error) {
...
// NewStateObject create a state object whkrypton it exist in the trie or not
...
func (s *StateDB) CommitBatch() (root common.Hash, batch krdb.Batch) {
	batch = s.db.NewBatch()
