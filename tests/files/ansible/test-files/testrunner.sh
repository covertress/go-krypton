...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-krypton/build/test/createRandomStateTest\" krypton/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pykrypton
	OUTPUT_PYTHON="$(docker run --rm krypton/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm krypton/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm krypton/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
