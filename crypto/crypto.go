...
import (
	...
	"github.com/krypton/go-krypton/crypto/secp256k1"
...
// Creates an krypton address given the bytes and the nonce
...
func decryptPreSaleKey(fileContent []byte, password string) (key *Key, err error) {
	...
	/*
		See https://github.com/krypton/pykrsaletool

		pykrsaletool generates the encryption key from password by
	...
	*/
	...
	krPriv := Sha3(plainText)
	ecKey := ToECDSA(krPriv)
	key = &Key{
		Id:         nil,
		Address:    PubkeyToAddress(ecKey.PublicKey),
		PrivateKey: ecKey,
	...
	expectedAddr := preSaleKeyStruct.KrAddr
