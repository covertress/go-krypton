...
func GetNotice(xkr *xkr.XKr, tx string, http *httpclient.HTTPClient) (notice string) {
	ns, err := New(xkr, tx, http)
	if err != nil {
		if ns == nil {
...
func New(xkr *xkr.XKr, jsontx string, http *httpclient.HTTPClient) (self *NatSpec, err error) {
	...
	content, err := FetchDocsForContract(contractAddress, xkr, http)
	if err != nil {
		return
	}
...
func FetchDocsForContract(contractAddress string, xkr *xkr.XKr, client *httpclient.HTTPClient) (content []byte, err error) {
	// retrieve contract hash from state
	codehex := xkr.CodeAt(contractAddress)
	codeb := xkr.CodeAtBytes(contractAddress)
...
func (self *NatSpec) makeAbi2method(abiKey [8]byte) (mkr *method) {
...
func (self *NatSpec) Notice() (notice string, err error) {
	...
	mkr := self.makeAbi2method(abiKey)

	if mkr == nil {
		err = fmt.Errorf("abi key does not match any method")
		return
	}
	notice, err = self.noticeForMethod(self.tx, mkr.name, mkr.Notice)
	return
}
