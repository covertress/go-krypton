...
func (self *testFrontend) ConfirmTransaction(tx string) bool {
	if self.wantNatSpec {
		...
		self.lastConfirm = GetNotice(self.xkr, tx, client)
	}
	return true
}
...
func testKr(t *testing.T) (krypton *kr.Krypton, err error) {
	...
	return kr.New(&kr.Config{
		DataDir:                 tmp,
		AccountManager:          am,
		Kryptonbase:               common.HexToAddress(testAddress),
		MaxPeers:                0,
		PowTest:                 true,
...
func testInit(t *testing.T) (self *testFrontend) {
	...
	addr, _ := self.krypton.Kryptonbase()
	...
	recG := self.xkr.GetTxReceipt(common.HexToHash(txG))
	if recG == nil {
	...
	recH := self.xkr.GetTxReceipt(common.HexToHash(txH))
	if recH == nil {
	...
	recU := self.xkr.GetTxReceipt(common.HexToHash(txU))
	if recU == nil {
