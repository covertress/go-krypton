...
const (
	...
	URN_WANKrnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANKrnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Krnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Krnet
//
...
// WANKrnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANKrnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANKrnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANKrnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANKrnetLinkConfig1Clients() (clients []*WANKrnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANKrnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANKrnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANKrnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANKrnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANKrnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANKrnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANKrnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANKrnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANKrnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANKrnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANKrnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANKrnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANKrnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANKrnetLinkConfig1 {
	clients := make([]*WANKrnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANKrnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewKrnetLinkStatus: allowed values: Up, Down
func (client *WANKrnetLinkConfig1) GetKrnetLinkStatus() (NewKrnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewKrnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANKrnetLinkConfig_1, "GetKrnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewKrnetLinkStatus, err = soap.UnmarshalString(response.NewKrnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
