...
const (
	...
	URN_WANKryptonnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANKryptonnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Kryptonnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Kryptonnet
//
...
// WANKryptonnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANKryptonnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANKryptonnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANKryptonnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANKryptonnetLinkConfig1Clients() (clients []*WANKryptonnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANKryptonnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANKryptonnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANKryptonnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANKryptonnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANKryptonnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANKryptonnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANKryptonnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANKryptonnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANKryptonnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANKryptonnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANKryptonnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANKryptonnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANKryptonnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANKryptonnetLinkConfig1 {
	clients := make([]*WANKryptonnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANKryptonnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewKryptonnetLinkStatus: allowed values: Up, Down
func (client *WANKryptonnetLinkConfig1) GetKryptonnetLinkStatus() (NewKryptonnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewKryptonnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANKryptonnetLinkConfig_1, "GetKryptonnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewKryptonnetLinkStatus, err = soap.UnmarshalString(response.NewKryptonnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
