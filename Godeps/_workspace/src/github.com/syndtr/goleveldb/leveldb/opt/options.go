...
type Options struct {
	...
	// ErrorIfExist defines whkrypton an error should returned if the DB already
	// exist.
	//
	// The default value is false.
	ErrorIfExist bool

	// ErrorIfMissing defines whkrypton an error should returned if the DB is
...
type ReadOptions struct {
	// DontFillCache defines whkrypton block reads for this 'read operation'
...
type WriteOptions struct {
	// Sync is whkrypton to sync underlying writes from the OS buffer cache
