/*
  This file is part of cpp-krypton.

  cpp-krypton is free software: you can redistribute it and/or modify
  it under the terms of the GNU General Public License as published by
  the Free Software Foundation, either version 3 of the License, or
  (at your option) any later version.

  cpp-krypton is distributed in the hope that it will be useful,
...
#include <libkrash/util.h>
#ifdef OPENCL
#include <libkrash-cl/krash_cl_miner.h>
#endif
#include <vector>
#include <algorithm>
...
static std::string bytesToHexString(krash_h256_t const *hash, unsigned size)
{
...
extern "C" int main(void)
{
...
#ifdef FULL
	{
		auto startTime = high_resolution_clock::now();
		krash_return_value hash;
		krash_full(&hash, full_mem, &params, &previous_hash, 0);
		auto time = std::chrono::duration_cast<std::chrono::milliseconds>(high_resolution_clock::now() - startTime).count();
		debugf("krash_full test: %uns\n", (unsigned)time);
	}
#endif
	...
	{
		krash_return_value hash;
		krash_light(&hash, &cache, &params, &previous_hash, i);
	...
	#ifdef OPENCL
	{
		struct search_hook : krash_cl_miner::search_hook
		{
			unsigned hash_count;
		...
		for (unsigned i = 0; i != hook.nonce_vec.size(); ++i)
		{
			...
			krash_return_value hash;
			krash_light(&hash, &cache, &params, &previous_hash, nonce);
	...
	#else
	{
		...
		for (int nonce = 0; nonce < trials; ++nonce)
		{
			krash_return_value hash;
			#ifdef FULL
				krash_full(&hash, full_mem, &params, &previous_hash, nonce);
			#else
				krash_light(&hash, &cache, &params, &previous_hash, nonce);
			#endif // FULL
		}
	...
	unsigned read_size = KRASH_ACCESSES * KRASH_MIX_BYTES;
