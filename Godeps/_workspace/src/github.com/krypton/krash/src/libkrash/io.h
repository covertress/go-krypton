...
// Reference: https://github.com/krypton/wiki/wiki/Krash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see krash_io_prepare
enum krash_io_rc {
	KRASH_IO_FAIL = 0,           ///< There has been an IO failure
	KRASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	KRASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	KRASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of krash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * krash_full_t
 */
#ifdef KRASH_PRINT_CRITICAL_OUTPUT
#define KRASH_CRITICAL(...)							\
	do													\
	{													\
		printf("KRASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for krash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the krash
 ...
 *                           https://github.com/krypton/wiki/wiki/Krash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum krash_io_rc
 */
 ...
 */
FILE* krash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* krash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool krash_mkdir(char const* dirname);
 ...
 */
bool krash_file_size(FILE* f, size_t* ret_size);
 ...
 *
 * The spec defining this directory is here: https://github.com/krypton/wiki/wiki/Krash-DAG
 *
 ...
 */
bool krash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool krash_io_mutable_name(
	uint32_t revision,
