...
/*
#include "src/libkrash/internal.h"

int krashGoCallback_cgo(unsigned);
*/
import "C"
...
import (
	...
	"github.com/krypton/go-krypton/pow"
)
...
func defaultDir() string {
	...
	if runtime.GOOS == "windows" {
		return filepath.Join(home, "AppData", "Krash")
	}
	return filepath.Join(home, ".krash")
}

// cache wraps an krash_light_t with some metadata
...
func (cache *cache) generate() {
	cache.gen.Do(func() {
		...
		size := C.krash_get_cachesize(C.uint64_t(cache.epoch * epochLength))
		if cache.test {
			size = cacheSizeForTesting
		}
		cache.ptr = C.krash_light_new_internal(size, (*C.krash_h256_t)(unsafe.Pointer(&seedHash[0])))
...
func freeCache(cache *cache) {
	C.krash_light_delete(cache.ptr)
	cache.ptr = nil
}
...
// Verify checks whkrypton the block's nonce is valid.
func (l *Light) Verify(block pow.Block) bool {
	// TODO: do krash_quick_verify before getCache in order
	...
	/* Cannot happen if block header diff is validated prior to PoW, but can
	   ...
	   Krypton protocol consensus rules here which are not in scope of Krash
	*/
	...
	dagSize := C.krash_get_datasize(C.uint64_t(blockNum))
	...
	ret := C.krash_light_compute_internal(cache.ptr, dagSize, hash, C.uint64_t(block.Nonce()))
	if !ret.success {
		return false
	}
...
func h256ToHash(in C.krash_h256_t) common.Hash {
	return *(*common.Hash)(unsafe.Pointer(&in.b))
}

func hashToH256(in common.Hash) C.krash_h256_t {
	return C.krash_h256_t{b: *(*[32]C.uint8_t)(unsafe.Pointer(&in[0]))}
}
...
// dag wraps an krash_full_t with some metadata
// and automatic memory management.
type dag struct {
	epoch uint64
	test  bool
...
func (d *dag) generate() {
	d.gen.Do(func() {
		var (
			...
			cacheSize = C.krash_get_cachesize(blockNum)
			dagSize   = C.krash_get_datasize(blockNum)
		)
		if d.test {
		...
		cache := C.krash_light_new_internal(cacheSize, (*C.krash_h256_t)(unsafe.Pointer(&seedHash[0])))
		defer C.krash_light_delete(cache)
		// Generate the actual DAG.
		d.ptr = C.krash_full_new_internal(
			C.CString(d.dir),
			hashToH256(seedHash),
			dagSize,
			cache,
			(C.krash_callback_t)(unsafe.Pointer(C.krashGoCallback_cgo)),
		)
		if d.ptr == nil {
			panic("krash_full_new IO or memory error")
		}
...
//export krashGoCallback
func krashGoCallback(percent C.unsigned) C.int {
...
func (pow *Full) Search(block pow.Block, stop <-chan struct{}, index int) (nonce uint64, mixDigest []byte) {
	...
	for {
		...
		default:
			...
			ret := C.krash_full_compute(dag.ptr, hash, C.uint64_t(nonce))
...
// Krash combines block verification with Light and
...
// created with New.
func New() *Krash {
	return &Krash{sharedLight, &Full{turbo: true}}
}
...
// regular-size cache.
func NewForTesting() (*Krash, error) {
	dir, err := ioutil.TempDir("", "krash-test")
	if err != nil {
		return nil, err
	}
	return &Krash{&Light{test: true}, &Full{Dir: dir, test: true}}, nil
}
