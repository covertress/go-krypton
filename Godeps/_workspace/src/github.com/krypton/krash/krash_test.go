...
func TestKrashVerifyValid(t *testing.T) {
	kr := New()
...
func TestKrashVerifyInvalid(t *testing.T) {
	kr := New()
	if kr.Verify(&invalidZeroDiffBlock) {
		t.Errorf("should not validate - we just ensure it does not panic on this block")
	}
}

func TestKrashConcurrentVerify(t *testing.T) {
	kr, err := NewForTesting()
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(kr.Full.Dir)

	block := &testBlock{difficulty: big.NewInt(10)}
	nonce, md := kr.Search(block, nil, 0)
	block.nonce = nonce
...
func TestKrashConcurrentSearch(t *testing.T) {
	kr, err := NewForTesting()
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < nsearch; i++ {
		go func() {
			nonce, md := kr.Search(block, stop, 0)
			select {
...
func TestKrashSearchAcrossEpoch(t *testing.T) {
	kr, err := NewForTesting()
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetSeedHash(t *testing.T) {
	...
	// From python:
	// > from pykrash import get_seedhash
	// > get_seedhash(30000)
