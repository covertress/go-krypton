...
/*
  ...
  1. The initCL(...)  function configures one or more OpenCL device
     (for now only GPU) and loads the Krash DAG onto device memory

  2. The Search(...) function loads a Krash nonce into device(s) memory and
     executes the Krash OpenCL kernel.
  ...
  1. https://github.com/krypton/wiki/wiki/Krash
  2. https://github.com/krypton/cpp-krypton/blob/develop/libkrash-cl/krash_cl_miner.cpp
...
type OpenCLDevice struct {
	...
	dagBuf        *cl.MemObject // Krash full DAG in device mem
...
type OpenCLMiner struct {
	...
	krash *Krash // Krash full DAG & cache in host mem
...
func PrintDevices() {
	...
	if len(found) == 0 {
		...
		fmt.Printf("Found %v devices. Benchmark first GPU:       gkr gpubench 0\n", len(found))
		fmt.Printf("Mine using all GPUs:                        gkr --minegpu %v\n", idsFormat)
	}
}
...
func InitCL(blockNum uint64, c *OpenCLMiner) error {
	platforms, err := cl.GetPlatforms()
	if err != nil {
		return fmt.Errorf("Plaform error: %v\nCheck your OpenCL installation and then run gkr gpuinfo", err)
	}
	...
	for _, p := range platforms {
		...
		if err != nil {
			return fmt.Errorf("Devices error: %v\nCheck your GPU drivers and then run gkr gpuinfo", err)
		}
		for _, d := range ds {
	...
	c.krash = pow
	dagSize := uint64(C.krash_get_datasize(C.uint64_t(blockNum)))
	c.dagSize = dagSize

	for _, id := range c.deviceIds {
		if id > len(devices)-1 {
			return fmt.Errorf("Device id not found. See available device ids with: gkr gpuinfo")
		} else {
...
func initCLDevice(deviceId int, device *cl.Device, c *OpenCLMiner) error {
	...
	kvs["DAG_SIZE"] = strconv.FormatUint(c.dagSize/krashMixBytesLen, 10)
	kvs["ACCESSES"] = strconv.FormatUint(krashAccesses, 10)
	...
	*/
	...
	searchKernelName = "krash_search"
	hashKernelName = "krash_hash"
	...
	dagPtr := unsafe.Pointer(c.krash.Full.current.ptr.data)
...
func (c *OpenCLMiner) Search(block pow.Block, stop <-chan struct{}, index int) (uint64, []byte) {
	c.mu.Lock()
	newDagSize := uint64(C.krash_get_datasize(C.uint64_t(block.NumberU64())))
		...
		if len(pending) == searchBufSize {
			...
			for i := uint32(0); i < nfound; i++ {
				...
				if checkNonce != 0 {
					...
					// executing the Krash verification function (on the CPU).
					ret := C.krash_light_compute_internal(c.krash.Light.current.ptr, ds, hashToH256(headerHash), cn)
...
func (c *OpenCLMiner) Verify(block pow.Block) bool {
	return c.krash.Light.Verify(block)
}
