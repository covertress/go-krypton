...
func Test_underscore_arrays_7(t *testing.T) {
  ...
  test("uniq", function() {
    ...
    equal(_.map(_.uniq(list, iterator), iterator).join(', '), 'moe, curly, larry', 'can find the unique values of an array using a custom iterator without specifying whkrypton array is sorted');
...
func Test_underscore_arrays_11(t *testing.T) {
  ...
  test('zip', function() {
    ...
    equal(String(stooges), 'moe,30,true,larry,40,,curly,50,', 'zipped togkrypton arrays of different lengths');
  });
        `)
	})
}
...
func Test_underscore_arrays_12(t *testing.T) {
  ...
  test('object', function() {
    ...
    ok(_.isEqual(result, shouldBe), 'two arrays zipped togkrypton into an object');

    result = _.object([['one', 1], ['two', 2], ['three', 3]]);
    shouldBe = {one: 1, two: 2, three: 3};
    ok(_.isEqual(result, shouldBe), 'an array of pairs zipped togkrypton into an object');
