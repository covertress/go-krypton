...
Set a Go function that returns somkring useful
...
Go translates JavaScript-style regular expressions into somkring that is "regexp" compatible via `parser.TransformRegExp`.
    ...
    func runUnsafe(unsafe string) {
        ...
        defer func() {
            ...
            if caught := recover(); caught != nil {
                ...
                panic(caught) // Somkring else happened, repanic!
            }
...
It would not be difficult to provide somkring like these via Go, but you probably want to wrap otto in an event loop in that case.
