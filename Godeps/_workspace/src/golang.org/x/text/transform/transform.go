...
type Transformer interface {
	...
	// atEOF argument tells whkr src represents the last bytes of the
	// input.
	//
	// Callers should always process the nDst bytes produced and account
	// for the nSrc bytes consumed before considering the error err.
	//
	// A nil error means that all of the transformed bytes (whkr freshly
	// transformed from src or left over from previous Transform calls)
	// were written to dst. A nil error can be returned regardless of
	// whkr atEOF is true. If err is nil then nSrc must equal len(src);
...
type Reader struct {
	...
	// transformComplete is whkr the transformation is complete,
	// regardless of whkr or not it was successful.
...
func (c *chain) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	...
	// The error returned by Transform determines whkr to increase or
