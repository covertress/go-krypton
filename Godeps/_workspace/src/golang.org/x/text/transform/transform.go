...
type Transformer interface {
	...
	// atEOF argument tells whkrypton src represents the last bytes of the
	// input.
	//
	// Callers should always process the nDst bytes produced and account
	// for the nSrc bytes consumed before considering the error err.
	//
	// A nil error means that all of the transformed bytes (whkrypton freshly
	// transformed from src or left over from previous Transform calls)
	// were written to dst. A nil error can be returned regardless of
	// whkrypton atEOF is true. If err is nil then nSrc must equal len(src);
...
type Reader struct {
	...
	// transformComplete is whkrypton the transformation is complete,
	// regardless of whkrypton or not it was successful.
...
func (c *chain) Transform(dst, src []byte, atEOF bool) (nDst, nSrc int, err error) {
	...
	// The error returned by Transform determines whkrypton to increase or
