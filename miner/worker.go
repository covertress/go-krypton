...
func newWorker(coinbase common.Address, kr core.Backend) *worker {
	worker := &worker{
		kr:            kr,
		mux:            kr.EventMux(),
		chainDb:        kr.ChainDb(),
...
func (self *worker) setKryptonbase(addr common.Address) {
	self.mu.Lock()
	...
	for {
		for result := range self.recv {
			...
			if self.fullValidation {
				...
				auxValidator := self.kr.BlockChain().AuxValidator()
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	state, err := state.New(parent.Root(), self.kr.ChainDb())
	if err != nil {
		return err
	}
	work := &Work{
	...
	accounts, _ := self.kr.AccountManager().Accounts()
...
func (self *worker) commitNewWork() {
	...
	/* //approach 1
	transactions := self.kr.TxPool().GetTransactions()
	...
	*/
	...
	for _, tx := range self.kr.TxPool().GetTransactions() {
	...
	*/
	...
	self.kr.TxPool().RemoveTransactions(work.lowGasTxs)
