...
type Miner struct {
	...
	canStart    int32 // can start indicates whkr we can start the mining operation
	shouldStart int32 // should start indicates whkr we should start after sync
}
...
func (self *Miner) SetKrbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setKrbase(addr)
}
