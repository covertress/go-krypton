...
// Package miner implements Krypton block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whkrypton we can start the mining operation
	shouldStart int32 // should start indicates whkrypton we should start after sync
}

func New(kr core.Backend, mux *event.TypeMux, pow pow.PoW) *Miner {
	miner := &Miner{kr: kr, mux: mux, pow: pow, worker: newWorker(common.Address{}, kr), canStart: 1}
	go miner.update()
...
func (self *Miner) SetKryptonbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setKryptonbase(addr)
}
