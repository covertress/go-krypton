...
func main() {
	...
	// Create an Krypton peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting Krypton peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start Krypton peer: %v.\n", err)
		os.Exit(1)
	}

	// Send a message to self to check that somkring works
